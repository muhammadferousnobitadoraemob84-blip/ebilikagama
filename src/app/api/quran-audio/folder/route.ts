...
// GET — Get the currently configured Quran Audio Google Drive folder
export async function GET() {
  try {
    ...
    const emailRecord = await withRetry(() =>
      prisma.setting.findUnique({ where: { key: "google_drive_email" } })
    );

    const connectedRecord = await withRetry(() =>
      prisma.setting.findUnique({ where: { key: "google_drive_connected" } })
    );
    ...
    return NextResponse.json(
      { connected: false, folderId: null, folderName: "" },
      { status: 200 }
    );
  }
}

// POST — Save the selected Google Drive folder ID
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "Failed to save folder: " + msg }, { status: 500 });
  }
}

// DELETE — Remove the configured folder (disconnect)
  ...
  try {
    ...
    await withRetry(() =>
      prisma.setting.deleteMany({
        where: {
          key: { in: ["quran_audio_folder_id", "quran_audio_folder_name"] },
        },
      })
    );
